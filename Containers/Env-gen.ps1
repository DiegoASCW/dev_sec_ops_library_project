...
docker-php-ext-install -j$(nproc) mysqli pdo_mysql >/dev/null 2>&1
docker-php-ext-configure gd --with-freetype --with-jpeg >/dev/null 2>&1
docker-php-ext-install -j$(nproc) gd >/dev/null 2>&1
docker-php-ext-enable mysqli pdo_mysql gd >/dev/null 2>&1
...
Write-Host "[DB] Loading schema and sample data as root"
($schema + "`n" + $sample) | docker exec -i mysql_stable mysql -u root -ppasswd openshelf
