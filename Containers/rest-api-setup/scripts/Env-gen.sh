...
if [[ "$escolha" == "y" ]]; then
  ...
  docker rm ubuntu_apache mysql_stable debian_api_gateway micro_auth_api micro_list_reg_books_api register_list_auth_api &> /dev/null || true
  # Remove all images
  docker rmi apache_openshelf_image mysql_openshelf_image debian_api_gateway_openshelf_image micro_auth_openshelf_image micro-list_reg_books_openshelf_image register-list-auth_openshelf_image -f &> /dev/null || true
  # Remove all networks
  docker network rm apache_network-R5 mysql_network-R4 \
    apache_mysql_network-R4-5 backup_mysql_network-R94 \
    api_gateway_apache_network-R1015 micro_auth_network_R1001 \
    micro_auth_mysql_network-R10014 micro_list_reg_books_network_R1002 \
    micro_list_reg_books_mysql_network-R10024 micro_register_list_auth_network_R1003 \
    micro_register_list_auth_mysql_network-R10034 &> /dev/null || true
  # Remove volumes
...
# Core Networks
docker network create --driver bridge --subnet=10.0.5.0/24 --gateway=10.0.5.254 apache_network-R5
docker network create --driver bridge --subnet=10.0.4.0/24 --gateway=10.0.4.254 mysql_network-R4
docker network create --driver bridge --subnet=10.0.45.0/24 --gateway=10.0.45.254 apache_mysql_network-R4-5
docker network create --driver bridge --subnet=10.0.94.0/24 --gateway=10.0.94.254 backup_mysql_network-R94

# API Gateway to Apache
docker network create --driver bridge --subnet=10.101.0.0/24 --gateway=10.101.0.254 api_gateway_apache_network-R1015

# Micro-Auth Service Networks
docker network create --driver bridge --subnet=10.100.1.0/24 --gateway=10.100.1.254 micro_auth_network_R1001
docker network create --driver bridge --subnet=10.100.14.0/24 --gateway=10.100.14.254 micro_auth_mysql_network-R10014

# Micro-List-Reg-Books Service Networks
docker network create --driver bridge --subnet=10.100.2.0/24 --gateway=10.100.2.254 micro_list_reg_books_network_R1002
docker network create --driver bridge --subnet=10.100.24.0/24 --gateway=10.100.24.254 micro_list_reg_books_mysql_network-R10024

# Micro-Register-List-Auth Service Networks
docker network create --driver bridge --subnet=10.100.3.0/24 --gateway=10.100.3.254 micro_register_list_auth_network_R1003
docker network create --driver bridge --subnet=10.100.34.0/24 --gateway=10.100.34.254 micro_register_list_auth_mysql_network-R10034


# -----------------------------
# 6. Apache/PHP container setup
# -----------------------------
echo -e "\n${BLUE}INFO${NC}: building and creating Apache 'ubuntu_apache' container..."
docker build -t apache_openshelf_image -f ../docker/apache/apache.dockerfile ../../..
docker create --name ubuntu_apache -p 80:80 -p 443:443 --env-file ../docker/apache/env.env apache_openshelf_image

echo -e "${BLUE}INFO${NC}: connecting networks to Apache container..."
...
echo -e "${BLUE}INFO${NC}: starting Apache container..."
docker start ubuntu_apache
docker restart ubuntu_apache # Restart to ensure all configs are applied

# -----------------------------
# 7. MySQL container setup
# -----------------------------
echo -e "\n${BLUE}INFO${NC}: building and creating MySQL 'mysql_stable' container..."
...
echo -e "${BLUE}INFO${NC}: connecting networks to MySQL container..."
...
echo -e "${BLUE}INFO${NC}: starting MySQL container..."
docker start mysql_stable

# -----------------------------
# 8. API Gateway container setup
# -----------------------------
echo -e "\n${BLUE}INFO${NC}: building and creating API Gateway 'debian_api_gateway' container..."
...
echo -e "${BLUE}INFO${NC}: connecting networks to API Gateway container..."
...
echo -e "${BLUE}INFO${NC}: starting API Gateway container..."
...
# -----------------------------
# 9. Micro-Auth service setup
# -----------------------------
echo -e "\n${BLUE}INFO${NC}: building and creating Micro-Auth 'micro_auth_api' container..."
...
echo -e "${BLUE}INFO${NC}: connecting networks to Micro-Auth container..."
...
echo -e "${BLUE}INFO${NC}: starting Micro-Auth container..."
docker start micro_auth_api

# -----------------------------
# 10. Micro-List-Reg-Books service setup
# -----------------------------
echo -e "\n${BLUE}INFO${NC}: building and creating Micro-List-Reg-Books 'micro_list_reg_books_api' container..."
...
echo -e "${BLUE}INFO${NC}: connecting networks to Micro-List-Reg-Books container..."
...
echo -e "${BLUE}INFO${NC}: starting Micro-List-Reg-Books container..."
docker start micro_list_reg_books_api

# -----------------------------
# 11. Micro-Register-List-Authors service setup
# -----------------------------
echo -e "\n${BLUE}INFO${NC}: building and creating Micro-Register-List-Authors 'register_list_auth_api' container..."
...
echo -e "${BLUE}INFO${NC}: connecting networks to Micro-Register-List-Authors container..."
...
echo -e "${BLUE}INFO${NC}: starting Micro-Register-List-Authors container..."
...
echo -e " - Apache server with PHP 8.2"
echo -e " - pdo_mysql extension and rewrite module enabled"
echo -e "\nTo access the container:"
echo -e "  docker exec -it ubuntu_apache bash"
echo -e "\nTo check Apache logs:"
echo -e "  docker exec -it ubuntu_apache bash -c 'tail -f /var/log/apache2/error.log'"
echo -e "\nTest PHP at: http://localhost/library and https://localhost/library"
...
echo -e "\nTo access MySQL:"
echo -e "  docker exec -it mysql_stable mysql -u root -p"

echo -e "\n${YELLOW}=========== Deployment Finished ===========${NC}\n"