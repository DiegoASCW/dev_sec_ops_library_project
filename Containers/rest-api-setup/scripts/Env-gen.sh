...
docker stop register_list_auth_api ubuntu_apache \
  mysql_stable debian_api_gateway \
  micro-auth-api micro_auth_api \
  micro_list_reg_books_api &> /dev/null || true

docker rmi register-list-auth_openshelf_image \
  debian_api_gateway_openshelf_image \
  micro-auth_openshelf_image \
  mysql_stable_image \
  apache_openshelf_image -f &> /dev/null || true
  
docker rm register_list_auth_api ubuntu_apache \
  mysql_stable debian_api_gateway micro-auth-api \
  micro_auth_api micro_list_reg_books_api &> /dev/null || true  
  
docker network rm apache_network-R5 mysql_network-R4 \
  apache_mysql_network-R45 apache_mysql_network-R4-5 openshelf_mysql_network-R4 \
  backup_mysql_network-R94 backup_mysql_network-R75 \
  backup_mysql_network-R74 micro_auth_network_R1001 \
  micro_register_list_auth_network_R1003 \
  micro_register_list_auth_mysql_network-R10034 \
...
docker network create --driver bridge --subnet=10.0.45.0/24 --ip-range=10.0.45.0/24 --gateway=10.0.45.254 apache_mysql_network-R45
...
echo -e "${BLUE}INFO${NC}: Apache/PHP environment created successfully!"
...
docker network connect --ip 10.0.45.10 apache_mysql_network-R45 mysql_stable
...
echo -e "${BLUE}INFO${NC}: preparing enviroment and installing dependencies"
docker build --platform=linux/amd64 -t debian_api_gateway_openshelf_image -f ../docker/api_gateway/api_gateway.dockerfile ../docker/api_gateway &> /dev/null || true
